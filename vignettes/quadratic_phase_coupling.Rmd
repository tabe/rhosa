...
Note that the trigonometric identities says
...
as the phase $\varphi_3$ is random.
...
```{r sec1_X1, fig.cap = "The first 256 observations of `x1`."}
plot(m1[,1], type = "l", ylim = c(-5, 5), xlab = "t", ylab = "value")
```
...
heatmap_bicoherence <- function(bc) {
    ggplot(bc) +
        geom_point(aes(f1, f2, colour = msbc), alpha = 0.5, size = 1) +
...
Note that the highest peak is at the bifrequency $(\frac{\lambda_5}{2 \pi},
\frac{\lambda_4}{2 \pi})$, which is approximately $(0.127, 0.095)$.
...
channels, say $C_1$ and $C_2$, we observe the summation of input and noises
as their output. On the other hand the last channel, called $C_3$, adds
$C_1$'s output multiplied by $C_2$'s to its own input and noise.
...
A cosine curve of yet another frequency for $C_3$'s input.
...
tc <- function(k) {
    ...
    function(x) {
        ...
        c3 <- i3(x, ps[3]) + rnorm(length(x), mean = 0, sd = 1) + Qcoef * c1 * c2
        data.frame(c1, c2, c3)
    }
}
...
plot(d1[,1], type = "l", ylim = c(-5, 5), xlab = "t", ylab = "value")
```

And $C_2$'s output:

```{r sec2_d2, fig.cap = "A sample path of `d2`.", fig.height = 4}
plot(d2[,1], type = "l", ylim = c(-5, 5), xlab = "t", ylab = "value")
```

$C_3$'s output:

```{r sec2_d3, fig.cap = "A sample path of `d3`.", fig.height = 4}
plot(d3[,1], type = "l", ylim = c(-5, 5), xlab = "t", ylab = "value")
```
...
Now we calculate the estimated cross-bicoherence of $C_1$, $C_2$, and $C_3$
by `cross_bicoherence`:

```{r sec2_cb1}
cb1 <- cross_bicoherence(d1, d2, d3)
```

... and define an R function to plot the heat map of the results:

```{r sec2_heatmap_cb1, fig.cap = "x1's estimated bicoherence", fig.height = 4}
...
heatmap_cross_bicoherence(cb1)
```

Notice the high values around bifrequencies $(\frac{0.9}{2 \pi}, \frac{0.6}{2 \pi})$
and $(\frac{0.9}{2 \pi}, -\frac{0.6}{2 \pi})$, which are approximately $(0.143,
0.095)$ and $(0.143, -0.095)$, respectively.
...
spectral analysis has been adopted already. The rhosa's functions lower the
barrier preventing from applying bispectrum analysis to time series for
